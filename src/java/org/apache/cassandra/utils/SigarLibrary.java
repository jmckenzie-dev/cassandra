        ...
        if (sigarInitialized)
        {
            ...
            if (swapEnabled || !goodAddressSpace || !goodFileLimits || !goodProcNumber)
            {
                logger.warn("Cassandra server running in degraded mode. Is swap disabled ? : {}  Address space adequate ? : {} " +
                            " nofile adequate ? : {} nproc adequate ? : {} ", swapEnabled, goodAddressSpace,
                            goodFileLimits, goodProcNumber );
            }
            else
            {
                logger.info("Checked swap, Address Space, number of open files per process and the number of processes per user and " +
                            "found them configured for optimal performance.");
            }
        }
        else
        {
