...
/**
 * CompressedRandomAccessReader ported to work with HDFS.
 * <p/>
 ...
 */
public class HadoopCompressedRandomAccessReader extends HadoopRandomAccessReader
{
    private final static Logger LOGGER = LoggerFactory.getLogger(CompressedRandomAccessReader.class);

    public static HadoopCompressedRandomAccessReader open(Path path, CompressionMetadata metadata, CompressedPoolingSegmentedFile owner, FileSystem fs)
    {
        try
        {
            return new HadoopCompressedRandomAccessReader(path, metadata, false, owner, fs);
        }
    ...
    public static HadoopCompressedRandomAccessReader open(Path dataFilePath, CompressionMetadata metadata, boolean skipIOCache, FileSystem fs)
    {
        try
        {
            return new HadoopCompressedRandomAccessReader(dataFilePath, metadata, skipIOCache, null, fs);
        }
        catch (FileNotFoundException e)
        {
            throw new RuntimeException(e);
    ...
    protected HadoopCompressedRandomAccessReader(Path dataFilePath,
