 ...
 * successfully.
 */
public class SSTableRewriter implements Transactional
{
    ...
    {
        if (!state.beginTransition(State.COMMITTED))
            return state.rejectedTransition(State.COMMITTED, accumulate);
        ...
        state.completeTransition(State.COMMITTED);
        return accumulate;
    }
    ...
    {
        for (Finished f : finishedWriters)
            accumulate = f.writer.cleanup(accumulate);
        return accumulate;
    }
