...
/**
 * Experimental meter with geometric rate-array growth. The independent implementation
 * keeps {@link ThreadLocalMeter} available for exact compatibility comparisons.
 */
public class GeometricThreadLocalMeter extends com.codahale.metrics.Meter implements Meter
{
    ...
    private static final List<WeakReference<GeometricThreadLocalMeter>> allMeters = new CopyOnWriteArrayList<>();
    ...
    static
    {
        backgroundTickingJob = ScheduledExecutors.scheduledTasks.scheduleWithFixedDelay(GeometricThreadLocalMeter::tickAll,
    ...
    private static int allocateRateGroupOffset()
    {
        ...
        synchronized (ratesArrayGuard)
        {
            ...
            {
                double[] newRates = new double[grownCapacity(rates.length, rateGroupId + RATES_COUNT)];
    ...
    {
        return (int) Math.min(Integer.MAX_VALUE, Math.max((long) required, 2L * current));
    }

    @VisibleForTesting
    static int rateCapacity()
    {
        return rates.length;
    }
    ...
    public GeometricThreadLocalMeter()
    {
    ...
    static void tickAll()
    {
            List<WeakReference<GeometricThreadLocalMeter>> emptyRefsToRemove = null;
            for (WeakReference<GeometricThreadLocalMeter> threadLocalMeterRef : allMeters)
            {
                GeometricThreadLocalMeter meter = threadLocalMeterRef.get();
                if (meter != null)
                {
