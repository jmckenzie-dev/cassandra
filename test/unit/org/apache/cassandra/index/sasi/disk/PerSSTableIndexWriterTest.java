...
public class PerSSTableIndexWriterTest extends SchemaLoader
{
    ...
    public void testPartialIndexWrites() throws Exception
    {
        ...
        OnDiskIndex index = new OnDiskIndex(new File(indexFile), Int32Type.instance, new KeyFetcher()
        {
            public DecoratedKey getPartitionKey(long offset)
            {
                ByteBuffer key = ByteBufferUtil.bytes(String.format(keyFormat, offset));
                return cfs.metadata().partitioner.decorateKey(key);
            }

            public Clustering getClustering(long offset)
            {
                return null; // TODO: test that as well
            }

            public RowKey getRowKey(DecoratedKey key, long rowOffset)
            {
                return null; // TODO: test that, too
            }
        });
    ...
    private static void populateSegment(TableMetadata metadata, PerSSTableIndexWriter.Index index, Map<Long, Set<Integer>> data)
    {
        for (Map.Entry<Long, Set<Integer>> value : data.entrySet())
        {
            ...
            for (Integer keyPos : value.getValue())
            {
                ...
                int partitionOffset = ThreadLocalRandom.current().nextInt(Integer.MAX_VALUE - 1);
                // TODO: row offset: is it reasonable?
                index.add(term, metadata.partitioner.decorateKey(key), partitionOffset, partitionOffset + 10);
            }
        }
    }
}
